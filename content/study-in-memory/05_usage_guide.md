...
### 【建议 0】延长 Session Reset 时间（全局配置）

**适用场景**：所有不想频繁被自动 reset 打断的对话

**操作方法**：
在 `openclaw.json` 中配置或使用 CLI：
```bash
openclaw config set session.resetByType.dm.idleMinutes 10080      # 7天
openclaw config set session.resetByType.thread.idleMinutes 10080  # 7天
```
